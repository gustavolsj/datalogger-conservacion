...
void setup()
{
  ...
  { // Si la tarjeta SD no funciona correctamente imprimimos el siguiente mensaje
  ...
  if (!rtc.begin())
  { // Si el módulo de reloj no inicia correctamente imprimimos el siguiente mensaje
    Serial.println("Error en el inicio del reloj");
    while (1)
      ; // Y el programa se detiene hasta aquí
  }

  if (!rtc.isrunning())
  { // Si el módulo de reloj inicia pero no se ejecuta correctamente imprimimos el siguiente mensaje
...
void loop()
{
  ...
  if (myFile)
  { // Si el archivo abre correctamente imprimimos el siguiente mensaje
  ...
  { // Si el archivo no abrió correctamente imprimimos el siguiente mensaje
